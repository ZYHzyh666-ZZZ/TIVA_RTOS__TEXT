...
float ano_x = 0,ano_x_old = 0;
float ano_y = 0,ano_y_old = 0;
float time_old = 0;

//调用一次time_ms累加，成功读取数据时，清零对应的计数。
...
static void AnoOF_DataAnl(uint8_t *data, uint8_t len)
{
	...
	if (*(data + 2) == 0X51) //光流信息
	{
		...
		else if (*(data + 4) == 2) //惯导融合后光流信息
		{
			...
			//积分过程
			if(ano_of.of_quality > 210)
			{
				ano_x_old = ano_of.of2_dx_fix;
				ano_y_old = ano_of.of2_dy_fix;
			}			
			ano_x += (Work_time - time_old) * ano_x_old;
			ano_y += (Work_time - time_old) * ano_y_old;

			time_old = Work_time;

			// ANO_DT_Send_MY_DATA(0xF2,2,(s16)(ano_x),(s16)(ano_y));
			//end
		}
	}
